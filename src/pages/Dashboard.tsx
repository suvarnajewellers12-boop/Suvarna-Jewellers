...
const getSchemeDetails = (scheme: Scheme) => {
  const enrolledDate = new Date(scheme.enrolledDate);
  const lastPaymentDate = new Date(enrolledDate);
  lastPaymentDate.setMonth(lastPaymentDate.getMonth() + scheme.paidMonths - 1);

  const nextDueDate = new Date(enrolledDate);
  nextDueDate.setMonth(nextDueDate.getMonth() + scheme.paidMonths);

  const isCompleted = scheme.paidMonths >= scheme.totalMonths;
      ...
      >
        ...
        <div className="px-8 pb-6">
          ...
          <p className="font-body text-sm text-muted-foreground text-center">
            {scheme.paidMonths} of {scheme.totalMonths} installments completed
          </p>
        </div>
        ...
        <div className="px-8 pb-6 grid grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-xl bg-gold/5 border border-gold/15">
            ...
            <p className="font-display text-lg font-bold text-gold-gradient">{formatINR(scheme.monthlyAmount * scheme.paidMonths)}</p>
          </div>
          <div className="p-4 rounded-xl bg-gold/5 border border-gold/15">
        ...
        <div className="px-8 pb-6">
          ...
          <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
            {Array.from({ length: scheme.totalMonths }, (_, i) => {
              const isPaid = i < scheme.paidMonths;
              const isCurrent = i === scheme.paidMonths;
              return (
...
const Dashboard = () => {
  ...
  const hasDuePayments = enrolledSchemes.some(
    (s) => s.paidMonths < s.totalMonths
  );
                  ...
                  >
                    ...
                    {isCompleted && (
                      <div className="absolute inset-0 rounded-2xl pointer-events-none"
                        style={{ background: 'radial-gradient(ellipse at 50% 50%, hsla(160, 50%, 50%, 0.06) 0%, transparent 60%)' }}
                      />
                    )}
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        {isDue && (
                          <div className="w-2 h-2 rounded-full animate-glow-pulse" style={{ background: 'hsl(var(--gold))' }} />
                        )}
                        <span className={`px-2 py-0.5 rounded-full text-xs font-body font-semibold border ${
                          isCompleted
                            ? "bg-emerald/10 text-emerald border-emerald/20"
                            : "bg-gold/10 text-gold-dark border-gold/20"
                        }`}>
                    ...
                    <p className="font-body text-xs text-muted-foreground mt-2">
                      {isCompleted ? "✦ Scheme completed" : `${scheme.totalMonths - scheme.paidMonths} months remaining`}
                    </p>
                  </motion.div>
                );
              })}
